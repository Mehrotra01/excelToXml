...
export const inputRowSchema = z.object({
  ...
  rcpType: z.array(z.string()).nonempty('rcpType must be a non-empty array of strings'),
  srtKey: z.object({
    level1: z.string().refine(val => val !== '', 'LEVEL1 is required'),
    level2: z.string().refine(val => val !== '', 'LEVEL2 is required'),
    level3: z.string().refine(val => val !== '', 'LEVEL3 is required'),
  }),
});
